...
const aluno1 = new Aluno("Daniela", 3, [9, 6, 2, 10]);
const aluno2 = new Aluno("Rayssa", 1, [7, 8, 9, 9]);
const aluno3 = new Aluno("Werlon", 2, [9, 10, 8.5, 7.5]);
const aluno4 = new Aluno("Rafaela", 1, [5, 7, 8, 8]);
const aluno5 = new Aluno("Andrew", 0, [5, 7, 6, 5]);

// TESTANDO A FUNÇÃO CONSTRUTORA DAS MEDIA E DAS FALTAS
...
let curso = {
  ...
  /* Passo 5 (2 pontos):
  Crie um método para o objeto curso que receba um aluno (como parâmetro) e retorne true se ele aprovou no curso ou false em caso de reprovação. Para ser aprovado, o aluno tem que ter uma média igual ou acima da nota de aprovação e ter menos faltas que faltas máximas. Se tiver a mesma quantidade de faltas permetidas, tem que estar 10% acima da nota de aprovação. */

  consultaAprovacaoAluno: function(aluno){
  ...
  resultadoBooleanoAluno: function () {
    var resultado = [];
    for (let i = 0; i < this.estudantes.length; i++) {
      var aluno = this.estudantes[i].nome;
      resultado.push(aluno);
      resultado.push(this.consultaAprovacaoAluno(this.estudantes[i]));
    }
    return resultado
  },
};

console.log(curso.criarAluno("Miguel", 0, [10, 7, 8, 9]));

console.log(curso.resultadoBooleanoAluno());
